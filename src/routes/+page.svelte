...
<svelte:head>
  ...
  <meta name="description" content="Bienvenida a la experiencia de Pache, una forma de vivir la vida de mi amada abuela." />
</svelte:head>

<main class="bg-neutral-950 w-full h-screen flex flex-col items-center justify-center text-white font-raleway">
    <div class="z-50 flex flex-col w-full justify-center items-center">
      ...
      <form class="flex flex-col items-center justify-center mt-8 relative max-w-xs bg-transparent" method="POST" action="?/exchangeCode">
        <div class="absolute w-40 h-12 rounded-full bg-neutral-400 blur-2xl bg-opacity-70"></div>
        <input name="code" type="text" class="bg-neutral-800 text-white p-2 rounded-md w-full text-center card outline-0 text-sm md:text-base capitalize" placeholder="0B1C2A"
           on:input={(e) => {
